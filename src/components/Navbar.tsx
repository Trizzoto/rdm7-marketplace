    ...
    <nav className="border-b border-[var(--border)] bg-[var(--surface)]/80 backdrop-blur-md sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-[68px]">
        <div className="flex items-center gap-8">
          ...
          <div className="hidden sm:flex items-center gap-6">
            ...
            <Link href="/browse?type=dbc" className={navLinkClass}>
              DBC Files
            </Link>
            <Link href="https://www.rdm7.com.au" target="_blank" className={navLinkClass}>
              Shop
            </Link>
          </div>
        </div>
      ...
      {mobileMenuOpen && (
        <div className="sm:hidden border-t border-[var(--border)] bg-[var(--surface)] px-4 py-4 space-y-3">
          ...
          <Link href="/settings" className="block text-sm font-medium text-[var(--text-muted)] hover:text-[var(--text)]" onClick={() => setMobileMenuOpen(false)}>
            Settings
          </Link>
          <button onClick={handleLogout} className="block text-sm font-medium text-[var(--text-muted)] hover:text-[var(--text)]">
            Sign Out
          </button>
        </div>
      )}
